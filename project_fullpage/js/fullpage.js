    ...
    console.log("pages: "+ pages);//pages: [object NodeList]
    ...
    */
    ...
    window.addEventListener("wheel", (e) => {
        ...
        if (scrollTop > lastPageTop) {
            ...
            // 양수면 아래로, 음수면 위로
            if (e.deltaY < 0) {
            //e.preventDefault():기본이벤트(wheel이벤트) 방지
                e.preventDefault();
        ...
        /* 일반 풀페이지 */
        e.preventDefault();
        //isScrolling= true이면 wheel이벤트 종료
        if (isScrolling) return;
        isScrolling = true;
        ...
        moveTo(currentPage);
        // setInterval(함수,시간): 매 시간 마다 반복
        // clearInterval()
        // setTimeout(함수,시간): 시간 이후 1번만 실행
        // clearTimeout()
        setTimeout(() => isScrolling = false, 600);
